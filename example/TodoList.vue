...
const dropItem = (dropItem: MovingItem<ItemType>) => {
  ...
  if (!dropItem.hoverElement?.value) return;
  ...
  if (dropItem.destination === trashBin) {
    // transition styles:
    const bin = trashBinElement.value?.getBoundingClientRect();
    if (bin && dropItem.hoverElement) {
      const hover = dropItem.hoverElement?.value.getBoundingClientRect();
      Object.assign(dropItem.hoverElement?.value.style, {
        top: `${(bin.bottom + bin.y - hover.bottom + hover.y) / 2}px`,
        left: `${(bin.right + bin.x - hover.right + hover.x) / 2}px`,
        scale: 0,
      });
    }
    // logic:
