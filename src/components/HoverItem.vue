...
onMounted(() => {
  ...
  watch(
    () => relativeMousePosition.value.ratio,
    (after, before) => {
      if (
        after.y > 1 - props.margin &&
        after.y < 1 &&
        (before.y > 1 || before.y < 1 - props.margin)
      ) {
        emit("mouseEnterBottom", relativeMousePosition.value);
      }
      if (
        after.y > 0 &&
        after.y < props.margin &&
        (before.y < 0 || before.y > props.margin)
      ) {
      ...
      if (
        after.y > props.margin &&
        after.y < 1 - props.margin &&
        (before.y < props.margin || before.y > 1 - props.margin)
      ) {
        emit("mouseEnterCenterY", relativeMousePosition.value);
      }
      if (
        after.x > 1 - props.margin &&
        after.x < 1 &&
        (before.x > 1 || before.x < 1 - props.margin)
      ) {
        emit("mouseEnterRight", relativeMousePosition.value);
      }
      if (
        after.x > 0 &&
        after.x < props.margin &&
        (before.x < 0 || before.x > props.margin)
      ) {
      ...
      if (
        after.x > props.margin &&
        after.x < 1 - props.margin &&
        (before.x < props.margin || before.x > 1 - props.margin)
      ) {
